...
def removeNull(CFG):
    ...
    for pre in CFG_product:
        ...
        for str_list in CFG_product[pre]:
            if str_list != [u'ε']:
                temp_product = []
                if per_remove_null(str_list,0,[],if_null_flag) == False:
                    continue
                #if temp_product != []:
        ...
        #将临时的temp覆盖原来的pre
        if temp == []:
            CFG['final_Production'].pop(pre)
            #if pre in CFG['Variable']:
            #    CFG['Variable'].remove(pre)
        else:
...
def per_remove_null(prod_list,index,rmv_null_str,if_null_flag):
    ...
    if index == len(prod_list):
        ...
        return True
    if prod_list[index] not in if_null_flag:
        return False
        #temp_product = []
        #return
        ...
        if pair[1] in CFG_product:
            for str_list in CFG_product[pair[1]]:
                if len(str_list) > 1 or (len(str_list) == 1 and str_list[0] in CFG['Terminal']):
                    m_pro.append(str_list)
        if m_pro != []:
            if pair[0] not in temp:
                temp[pair[0]] = []
            #temp[pair[0]] = copy.deepcopy(m_pro)
            temp[pair[0]] += m_pro
        ...
        for fore in CFG_product:
            if fore not in product_set:
                for str_list in CFG_product[fore]:
                    if str_list == [u'ε'] or if_prodiction(str_list,product_set) and fore not in product_set:
...
def remove_not_reachable(CFG):
    ...
    for ter in CFG['Terminal']:
        if ter not in reachable_set:
            CFG['Terminal'].remove(ter)
    temp_var = copy.deepcopy(CFG['Variable'])
    for var in temp_var:
...
def turn_CNF(CFG):
    ...
    CFG_product = copy.deepcopy(CFG['final_Production'])
    for fore in CFG_product:
        if len(CFG_product[fore]) == 1 and len(CFG_product[fore][0]) == 1 and CFG_product[fore][0][0] in CFG['Terminal'] and CFG_product[fore][0][0] not in ter_var_dict:
            ter_var_dict[CFG_product[fore][0][0]] = fore
    #为所有终结符配一个变元
    m_index = 0
    for ter in CFG['Terminal']:
